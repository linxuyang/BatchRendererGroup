...
public class TestBRG : MonoBehaviour
{
    ...
    private const int kSizeOfMatrix = sizeof(float) * 4 * 4; //矩阵字节数
    private const int kSizeOfPackedMatrix = sizeof(float) * 4 * 3; //矩阵字节数
    private const int kSizeOfFloat4 = sizeof(float) * 4;    //向量字节数
    private const int kBytesPerInstance = (kSizeOfPackedMatrix * 2) + kSizeOfFloat4; //一个实例的字节数（2个4*3矩阵+1个向量)
    private const int kExtraBytes = kSizeOfMatrix * 2; //额外字节
    private const int kNumInstances = 3; //实例数量
    ...
    private void PopulateInstanceDataBuffer()
    {
        ...
        // 计算不同实例属性的开始地址。unity_ObjectToWorld从96开始 而不是 64，这意味着 32 位未初始化。这是因为
        // computeBufferStartIndex 参数要求起始偏移量能被源大小整除
        // 数组元素类型。在本例中，它是 PackedMatrix 的大小，即 48。
        ...
        IntPtr userContext)
    {
        ...
        // 这里不使用Job，这复杂的系统中，可以使用Job进行剔除等操作

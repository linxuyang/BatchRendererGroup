...
//            float sat = 0.5f;
    ...
    {
        int totalGpuBufferSize; //buffer的总大小，就是window数*每个window尺寸
        int alignedWindowSize;//原始window尺寸
        ...
        UpdatePositionsJob myJob = new UpdatePositionsJob()
        {
            ...
            _maxInstancePerWindow = alignedWindowSize / kGpuItemSize, //每个窗口最大实例数
            _windowSizeInFloat4 = alignedWindowSize / 16,   //窗口的float4数
        };
